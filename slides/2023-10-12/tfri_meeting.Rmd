...
## TFRI P3: Characterizing the tumour microenvironment of p53abn EC

```{r, out.width = '60%', fig.align = 'center'}
knitr::include_graphics('images/tfri_p3_aims.png')
```
...
metatable_clinical <- clinical_data_unique %>%
  select(cohort, acc_num, age_dx, eclass2_ngs, stage_full, grade_rev, hist_rev, lvi, nodes, myo, neoadj, chemo, rt, brachy) %>%
  unique
...
cohort_plot_bottom <- columnAnnotation(df = metatable_full %>% select(-c(acc_num, TMA, `B & T`, Angiogenesis)) %>% as.data.frame, 
                                       col = list(
...
cohort_plot_top <- columnAnnotation(df = metatable_full %>% select(c(TMA, `B & T`, Angiogenesis)) %>% as.data.frame, 
                                    col = list(
                                      TMA=c("10-005"="#88A096",
                                            "10-006"="#BBAB8B",
                                            "22-004"="#8A4F7D"),
...
cyto_treg_ratios <- til_densities_output %>%
  ...
  select(acc_num, cyto_treg_ratio_tumour, cyto_treg_ratio_stroma) %>%
...
cnsig_exposures_til_densities <- cnsig_exposures %>%
  select(-c(max_van_sig, max_brenton_sig, patient_id, study_id)) %>%
