...
clust_assignments_map <- clust_assignments_summarized %>%
  ...
  mutate(
    clust_map = dplyr::recode(clust_map, '0'='High TIL', '1'='Low TIL') %>%
      factor(levels = c("Low TIL", "High TIL"))
  )
...
km_chemo_fits <- lapply(unique_outcomes, function(x) {
  tmp <- lapply(unique_chemo, function(chemo_opt) {
    ...
    plot <- ggsurvplot(fit,
                       ...
                       conf.int = TRUE,
                       risk.table = TRUE, 
                       title = paste0("Chemo ", chemo_opt, " (n=", length(unique(intersect(df$acc_num, metatable_full$acc_num))), ", ", str_replace_all(pval$pval.txt, " ", ""), ")"),
                       legend = "none",
                       palette = unname(palette_tilclust),
                       xlim = c(0, 13),
                       break.x.by = 3,
