...
col_anno <- (overall_annotations_comb_tma %>% 
               ...
               select(-c(frac_vessel_all, count_pct)) %>%
...
density_heatmap <- Heatmap(mat %>% t,
                           ...
                           column_split = col_anno$clust_map,
                           top_annotation = ha,
...
plot_grid(densities_tilclust_plot, cd8_treg_ratio_plot + theme(legend.position = "none"), nrow = 2, ncol = 1, rel_heights = c(0.6, 0.4), labels = c("a", "b"))
```
...
density_cormat <- density_final_input %>%
  cor(., method = "pearson")
diag(density_cormat) <- NA

pheatmap(density_cormat, color = c("white", "red"), breaks = c(0, 1), heatmap_legend_param = list(title = "Correlation"))
```
...
cox_tables <- lapply(cox_fits, function(cox_fit) {
  cox_table <- cox_fit %>% tbl_regression(exponentiate = TRUE, show_single_row = c("TIL", "Chemotherapy", "Radiotherapy", "Brachytherapy"))
...
plot_grid(top_plot, cluster_legend, nrow = 2, rel_heights = c(1, 0.1))
```
