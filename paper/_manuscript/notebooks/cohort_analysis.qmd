...
```{r}
cohort_plot_bottom <- columnAnnotation(df = metatable_full %>% dplyr::select(-c(acc_num, TMA, `B & T`, `Adaptive Resistance`)) %>% as.data.frame, 
                                       col = list(
...
summarizor(metatable_full %>% select(Cohort, Age, Stage, Myo, Nodes, Chemotherapy, Radiotherapy, Brachytherapy, Histotype, acc_num) %>% unique %>% select(-c(acc_num))) %>% flextable::as_flextable() %>% autofit()
```